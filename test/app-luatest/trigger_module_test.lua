...
g.test_del_during_pairs = function()
    ...
    local event = 'event'
    local trigger_num = 100
    local break_point = math.random(5, 95)
    ...
    local deleted_id = math.random(break_point + 1, 100)
...
g.test_invalid_args = function()
    ...
    local errmsg = 'event trigger set: incorrect arguments'
    for _, h in pairs(invalid_handlers) do
        t.assert_error_msg_content_equals(errmsg, trigger.set, event, name, h)
    end
    ...
    for _, invalid_name in pairs(invalid_names) do
        t.assert_error_msg_contains(
            errmsg, trigger.set, event, invalid_name, handler)
        t.assert_error_msg_contains(
